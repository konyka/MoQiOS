...
void _start(void) {
    ...
    if (syscall6(SYS_FUTEX, (uint64_t)page, FUTEX_WAIT | FUTEX_PRIVATE_FLAG, 0, 0, 0, 0) != -14) {
        fail("unmapped futex word EFAULT");
    }

    if (syscall2(SYS_FUTEX_WAITV, 0, 1) != -38) fail("waitv ENOSYS");
    if (syscall2(SYS_FUTEX_WAITV, 0, 17) != -38) fail("waitv oversized ENOSYS");

    const int64_t array_page = syscall6(SYS_MMAP, 0, PAGE, PROT_READ | PROT_WRITE,
                                        MAP_PRIVATE | MAP_ANONYMOUS, (uint64_t)-1, 0);
    if (array_page <= 0) fail("array mmap");
    if (syscall2(SYS_FUTEX_WAITV, (uint64_t)array_page + PAGE - 16, 1) != -38) {
        fail("partial waiter record ENOSYS");
    }

    const int64_t ro = syscall6(SYS_MMAP, 0, PAGE, PROT_READ,
                                MAP_PRIVATE | MAP_ANONYMOUS, (uint64_t)-1, 0);
    if (ro <= 0) fail("readonly mmap");
    if (syscall6(SYS_FUTEX, (uint64_t)ro, FUTEX_WAIT | FUTEX_PRIVATE_FLAG, 1, 0, 0, 0) != -11) {
