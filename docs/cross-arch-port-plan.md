...
- **架构抽象**：`kernel/arch/arch.zig` 已存在（M4）；x86_64 / riscv64 / aarch64 各有 `arch_impl.zig`。
  **SK-8**…**SK-31**：facade、portable mm、时间片原生用户抢占、default timer fallthrough。
  **SK-32（2026-07-17）**：`sk_probes.runEarly`/`runPostMm` 统一非 x86 阶梯；
  `subsystem_boot.initSlab` 与 `main.zig` 对齐；探针
  `[SK-32] shared sk probes+slab boot: OK`。
...
Phase F — 第二 ISA              ✅ M2–M7；✅ SK-1…SK-32
Phase F2 — 第三 ISA             ✅ M9-7；✅ SK-1…SK-32
...
> 当前下一执行项：**SK-33** — 继续非 x86 引导向 `main.zig` 可复用片段收敛；
> SK-1…SK-32 已完成。
