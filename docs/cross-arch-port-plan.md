...
- **架构抽象**：`kernel/arch/arch.zig` 已存在（M4）；x86_64 / riscv64 / aarch64 各有 `arch_impl.zig`。
  **SK-6**：从内核镜像上方划出 4MiB 真实物理内存给共享 PMM。
  **SK-7（2026-07-11）**：40 个共享叶模块的 `serial` 直连全部迁到 `arch.serial`。
...
- **后续**：SK-8 继续 facade（tsc/paging）与驱动 comptime 隔离。
...
Phase F — 第二 ISA              ✅ M2–M7；✅ SK-1…SK-7；⬜ SK-8 tsc/paging facade
Phase F2 — 第三 ISA             ✅ M9-7；✅ SK-1…SK-7；⬜ SK-8 tsc/paging facade
...
> 当前下一执行项：**SK-8** — 继续把 `tsc`/paging 等直连迁到 facade，或 comptime 隔离 ACPI/PCI 以便更大子集链接；
> SK-1…SK-7 已完成。
