...
- **aarch64**：M9-1 骨架完成（`-Darch=aarch64`、PL011、QEMU virt 横幅）。
  门禁：`zig build -Darch=aarch64 smoke-aarch64`。后续：异常向量 / 分页 / 定时器。
- **引导**：x86_64 经 Limine；riscv64 经 OpenSBI `-kernel`（链接地址 `0x80200000`）；
  aarch64 经 QEMU `-kernel`（链接地址 `0x40000000`，EL1）。
...
| **M9** | aarch64 后端（复用 M4 抽象；先骨架再异常/分页/定时器/用户态） | QEMU aarch64：`M9-1 complete` → 后续用户态 | 🟡 M9-1 骨架完成（2026-07-11） |
...
### 3.11 M9-1 完成记录（2026-07-11，aarch64 骨架）

- **构建**：`-Darch=aarch64` → `moqi-kernel-aarch64.elf`；链接地址 `0x40000000`。
- **控制台**：PL011 @ `0x09000000`（QEMU virt）；打印横幅 + DTB 指针。
- **arch facade**：`aarch64/arch_impl.zig` 接入 `arch.zig`（serial 实装，其余 stub）。
- **门禁**：`zig build -Darch=aarch64 smoke-aarch64`（`M9 skeleton` + `M9-1 complete`）。
- **后续**：异常向量表、identity map、generic timer、U-mode（对标 riscv M2–M6）。
...
| **aarch64** | M9-1 骨架 ✅（PL011） | ✅ M9-1 | N/A（第三 ISA） |
...
Phase F2 — 第三 ISA             🟡 M9-1（PL011 骨架）；⬜ 异常/分页/定时器/U-mode
...
> 当前下一执行项：**M9-2 aarch64 异常向量**，或 **riscv64 共享内核复用**。
> M3–M7（blk+net）与 M9-1 已于 2026-07-11 完成。
