...
- **aarch64**：M9-1…M9-5 完成（…CNTV + GICv3 PPI IRQ）。
  门禁：`zig build -Darch=aarch64 smoke-aarch64`。后续：U-mode。
...
| **M9** | aarch64 后端（骨架 → 异常 → 分页 → 定时器 → GIC → 用户态） | QEMU aarch64：`M9-5 complete` → 后续用户态 | 🟡 M9-5 完成（2026-07-11） |
...
### 3.11 M9-1…M9-5 完成记录（2026-07-11，aarch64）

- **构建**：`-Darch=aarch64`（`baseline-neon`）→ ELF @ `0x40000000`；QEMU `virt,gic-version=3`。
- **M9-1…M9-4**：PL011、loader DTB、VBAR/`brk`、PMM+MMU+#PF、CNTV ISTATUS 轮询。
- **M9-5**：最小 GICv3（GICD @ `0x08000000` / GICR @ `0x080A0000`）+ ICC_*；
  CNTV PPI 27 → EL1 IRQ；`timer IRQ firings=3`。
- **门禁**：`smoke-aarch64`（…`timer IRQ firings=` + `M9-5 complete`）。
- **后续**：U-mode（对标 riscv M6）。
...
| **aarch64** | M9-1…M9-5 ✅（…GIC + timer IRQ） | ✅ M9-5 | N/A（第三 ISA） |
