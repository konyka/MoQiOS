...
### 3.158 COW 克隆保留 NX（2026-07-25）

- **背景**:接手 3.157 刻意留下的一项——COW 克隆以 `phys | (pte & 0xFFF)` 重建子进程的
  页表项,而 `NX` 是 bit 63,位于地址字段之上而非低位权限位之中,于是被掩掉。
  修复前实测（统计父页带 `NX` 而子页不带的项）:
  `parent_pte=0x80000000002e4067` → `child_pte=0x00000000002e4265`,`lost=3 kept=0`
  ——**没有一个不可执行页在 fork 后存活**。每个子进程都拿到既可写又可执行的栈和堆;
  这些映射创建时明确带 `no_execute = true`（`mmap`/`brk`/栈都是）,但 W^X 只维持到进程
  fork 为止。`EFER.NXE` 已开启（实测 `EFER=0xd01`),因此这是真实的保护失效,而非仅仅
  标志位不一致。
  - **P1** 父子两侧本应持有**同一个**页表项,而父侧写作 `(pte & ~WRITABLE) | COW`
    （保留高位）、子侧从零件重建,两侧因此不一致,且只有重建的那侧是错的。
- **方案**:这种不对称本身就是病因,所以修法是消除它而不是给第二个表达式补上丢失的位。
  新增 `kernel/mm/cow_pte.zig`——一个不含任何 import 的小模块,承载该推导,同时因此可
  在宿主上做单元测试;`fork.zig` 的 `cloneUserPagesCow` 与 `clone.zig` 的
  `cloneUserPages` 两处共用它。COW 位的另外两个读取方（`handleCowFault` 两条分支）
  本来就是对的:它们只掩掉地址字段和 COW 标记,`NX` 在缺页处理中得以保留。
- **效果**:fork 出的子进程不再获得可执行的栈与堆,W^X 跨 fork 成立。修复后实测
  `lost=0 kept=3`,与修复前正好相反。
- **验证**:三架构构建 + `zig build test` + `smoke`/`smoke-smp`/`smoke-smp-stress` +
  riscv64/aarch64 smoke 全绿。单元测试覆盖:`NX` 得以保留、物理帧以只读方式共享且置上
  COW 标记、present/user 保留、对已共享项再克隆是幂等的。其中一项直接表述该回归——断言
  低 12 位重建会丢掉 `NX` 而 `cowPte` 不会。负面对照:把旧推导塞回 `cowPte` 内部,该项
  测试失败。
- **已知边界**:端到端的**执行**语义未验证,即"fork 出的子进程从数据页取指必须失败"。
  障碍在于内核遇到用户态故障是直接杀进程而非投递 `SIGSEGV`,测试程序无法在故障后存活
