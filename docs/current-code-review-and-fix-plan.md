...
> Last update: 2026-07-23 (SK-102 interface/RA MTU → SYN MSS + prior SK-101 reviewed and verified)
...
| `zig build smoke-smp` | Passed | Same markers with `MOQI_SMP=2`. |
| `zig build -Darch=riscv64 smoke-riscv` | Passed | Includes `[SK-102] if mtu ra syn mss non-x86: OK`. |
| `zig build -Darch=aarch64 smoke-aarch64` | Passed | Includes `[SK-102] if mtu ra syn mss non-x86: OK`. |
