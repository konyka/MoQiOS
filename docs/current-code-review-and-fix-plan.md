...
> Last update: 2026-07-22 (SK-81 NDP delay/probe unicast NUD + prior SK-80 reviewed and verified)
...
| `zig build smoke-smp` | Passed | Same markers with `MOQI_SMP=2`. |
| `zig build -Darch=riscv64 smoke-riscv` | Passed | Includes `[SK-81] ndp delay probe unicast non-x86: OK`. |
| `zig build -Darch=aarch64 smoke-aarch64` | Passed | Includes `[SK-81] ndp delay probe unicast non-x86: OK`. |
