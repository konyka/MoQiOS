...
> Last update: 2026-07-24 (SK-126 RACK per-segment + prior SK-125 reviewed and verified)
...
| `zig build smoke-smp` | Passed | Same markers with `MOQI_SMP=2`. |
| `zig build -Darch=riscv64 smoke-riscv` | Passed | Includes `[SK-126] tcp rack per-segment non-x86: OK`. |
| `zig build -Darch=aarch64 smoke-aarch64` | Passed | Includes `[SK-126] tcp rack per-segment non-x86: OK`. |
