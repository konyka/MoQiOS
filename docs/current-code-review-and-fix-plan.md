...
> Last update: 2026-07-23 (SK-89 router lifetime aging + prior SK-88 reviewed and verified)
...
| `zig build smoke-smp` | Passed | Same markers with `MOQI_SMP=2`. |
| `zig build -Darch=riscv64 smoke-riscv` | Passed | Includes `[SK-89] ndp router lifetime aging non-x86: OK`. |
| `zig build -Darch=aarch64 smoke-aarch64` | Passed | Includes `[SK-89] ndp router lifetime aging non-x86: OK`. |
