...
> Last update: 2026-07-22 (SK-80 NDP reachable→stale aging + prior SK-79 reviewed and verified)
...
| `zig build smoke-smp` | Passed | Same markers with `MOQI_SMP=2`. |
| `zig build -Darch=riscv64 smoke-riscv` | Passed | Includes `[SK-80] ndp reachable stale aging non-x86: OK`. |
| `zig build -Darch=aarch64 smoke-aarch64` | Passed | Includes `[SK-80] ndp reachable stale aging non-x86: OK`. |
