...
| `zig build -Darch=riscv64 smoke-riscv` | riscv64 M7+SK-5：共享 pmm+slab + virtio + U-mode + sched |
| `zig build -Darch=aarch64 smoke-aarch64` | aarch64 M9-7+SK-5：共享 pmm+slab + preempt + `M9-7 complete` |
