...
| `zig build -Darch=riscv64 smoke-riscv` | riscv64 M7+SK-92：shared probes + slim BSS + shared user-copy guard + shared idle boot + shared ramdisk parse + virtio + U-mode |
| `zig build -Darch=aarch64 smoke-aarch64` | aarch64 M9-7+SK-92：shared probes + slim BSS + shared user-copy guard + shared idle boot + shared ramdisk parse + default timer + EL0/SVC |
