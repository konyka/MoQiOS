...
| `zig build -Darch=riscv64 smoke-riscv` | riscv64 M7+SK-9：完整 arch facade + virtio + U-mode + sched |
| `zig build -Darch=aarch64 smoke-aarch64` | aarch64 M9-7+SK-9：完整 arch facade + preempt + `M9-7 complete` |
