...
while [ "$SECONDS" -lt "$deadline" ]; do
    if pass_markers; then
        echo "PASS: MoQiOS riscv64 M7+SK-125 smoke (shared probes + slim footprint/env + virtio + U-mode)."
...
echo "Expected: SK-2..SK-4 + SK-6..SK-125 shared markers + M7 blk/net + M6 + M5 markers."
echo "QEMU log: $RUN_LOG"
echo "Serial log: $LOG_FILE"
exit 1
