...
while [ "$SECONDS" -lt "$deadline" ]; do
    if pass_markers; then
        echo "PASS: MoQiOS aarch64 M9-7+SK-151 smoke (shared probes + slim footprint/env + default timer + EL0/SVC)."
...
echo "Expected: SK-2..SK-4 + SK-6..SK-151 shared markers + M9-1..M9-7 markers."
echo "QEMU log: $RUN_LOG"
echo "Serial log: $LOG_FILE"
exit 1
