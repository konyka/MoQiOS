...
while [ "$SECONDS" -lt "$deadline" ]; do
    ...
    if [ -f "$LOG_FILE" ] &&
       ...
       grep -q "hello39: PASS (socket option faults/address lengths)" "$LOG_FILE" &&
        ...
        ! grep -q "hello71: FAIL" "$LOG_FILE" &&
        grep -q "hello71: PASS" "$LOG_FILE" &&
        grep -q "hello71 done" "$LOG_FILE" &&
         ! grep -q "hello72: FAIL" "$LOG_FILE" &&
         grep -q "hello72: PASS" "$LOG_FILE" &&
         grep -q "hello72 done" "$LOG_FILE" &&
         ! grep -q "hello73: FAIL" "$LOG_FILE" &&
         grep -q "hello73: PASS" "$LOG_FILE" &&
         grep -q "hello73 done" "$LOG_FILE" &&
          ! grep -q "hello74: FAIL" "$LOG_FILE" &&
         grep -q "hello74: PASS" "$LOG_FILE" &&
         grep -q "hello74 done" "$LOG_FILE" &&
         ! grep -q "hello75: FAIL" "$LOG_FILE" &&
         grep -q "hello75: PASS" "$LOG_FILE" &&
         grep -q "hello75 done" "$LOG_FILE" &&
         ...
         ! grep -q "hello78: FAIL" "$LOG_FILE" &&
         grep -q "hello78: PASS" "$LOG_FILE" &&
         grep -q "hello78 done" "$LOG_FILE" &&
          ! grep -q "hello79: FAIL" "$LOG_FILE" &&
          grep -q "hello79: PASS" "$LOG_FILE" &&
          grep -q "hello79 done" "$LOG_FILE" &&
    ...
    grep -q "hello80 done" "$LOG_FILE" &&
     ...
     ! grep -q "hello84: FAIL" "$LOG_FILE" &&
     grep -q "hello84: PASS" "$LOG_FILE" &&
        grep -q "hello84 done" "$LOG_FILE" &&
        ! grep -q "hello85: FAIL" "$LOG_FILE" &&
        grep -q "hello85: PASS" "$LOG_FILE" &&
        grep -q "hello85 done" "$LOG_FILE" &&
        ! grep -q "hello86: FAIL" "$LOG_FILE" &&
        grep -q "hello86: PASS" "$LOG_FILE" &&
        grep -q "hello86 done" "$LOG_FILE" &&
        ! grep -q "hello87: FAIL" "$LOG_FILE" &&
        grep -q "hello87: PASS" "$LOG_FILE" &&
        grep -q "hello87 done" "$LOG_FILE" &&
         ! grep -q "hello88: FAIL" "$LOG_FILE" &&
         grep -q "hello88: PASS" "$LOG_FILE" &&
         grep -q "hello88 done" "$LOG_FILE" &&
          ! grep -q "hello89: FAIL" "$LOG_FILE" &&
          grep -q "hello89: PASS" "$LOG_FILE" &&
          grep -q "hello89 done" "$LOG_FILE" &&
          ! grep -q "hello90: FAIL" "$LOG_FILE" &&
         grep -q "hello90: PASS" "$LOG_FILE" &&
         grep -q "hello90 done" "$LOG_FILE" &&
           ! grep -q "hello91: FAIL" "$LOG_FILE" &&
           grep -q "hello91: PASS" "$LOG_FILE" &&
          grep -q "hello91 done" "$LOG_FILE" &&
           ! grep -q "hello92: FAIL" "$LOG_FILE" &&
           grep -q "hello92: PASS" "$LOG_FILE" &&
          grep -q "hello92 done" "$LOG_FILE" &&
           ! grep -q "hello93: FAIL" "$LOG_FILE" &&
           grep -q "hello93: PASS" "$LOG_FILE" &&
          grep -q "hello93 done" "$LOG_FILE" &&
           ! grep -q "hello94: FAIL" "$LOG_FILE" &&
           grep -q "hello94: PASS" "$LOG_FILE" &&
          grep -q "hello94 done" "$LOG_FILE" &&
           ! grep -q "hello95: FAIL" "$LOG_FILE" &&
           grep -q "hello95: PASS" "$LOG_FILE" &&
          grep -q "hello95 done" "$LOG_FILE" &&
           ! grep -q "hello96: FAIL" "$LOG_FILE" &&
           grep -q "hello96: PASS" "$LOG_FILE" &&
          grep -q "hello96 done" "$LOG_FILE" &&
           ! grep -q "hello97: FAIL" "$LOG_FILE" &&
           grep -q "hello97: PASS" "$LOG_FILE" &&
          grep -q "hello97 done" "$LOG_FILE" &&
           ! grep -q "hello98: FAIL" "$LOG_FILE" &&
           grep -q "hello98: PASS" "$LOG_FILE" &&
          grep -q "hello98 done" "$LOG_FILE" &&
           ! grep -q "hello99: FAIL" "$LOG_FILE" &&
           grep -q "hello99: PASS" "$LOG_FILE" &&
          grep -q "hello99 done" "$LOG_FILE" &&
           ! grep -q "hello100: FAIL" "$LOG_FILE" &&
           grep -q "hello100: PASS" "$LOG_FILE" &&
          grep -q "hello100 done" "$LOG_FILE" &&
     ...
     grep -q "hello82: PASS" "$LOG_FILE" &&
     grep -q "hello82 done" "$LOG_FILE" &&
