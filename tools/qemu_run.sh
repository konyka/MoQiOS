...
if [ "${MOQI_AHCI:-1}" != "0" ]; then
    if [ ! -f "$AHCI_IMAGE" ]; then
        # 256 MiB sparse: also serves as the hello96 swap target — sized to
        # the swap slot limit (65536 pages) so parallel reclaim overshoot on
        # SMP=4 can never exhaust slots mid-churn.
        truncate -s 256M "$AHCI_IMAGE"
    fi
