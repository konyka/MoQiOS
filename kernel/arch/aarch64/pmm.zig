//! Physical page allocator for the aarch64 skeleton (Milestone 9-3).
//!
//! Intrusive freelist of 4 KiB pages. Usable RAM comes from the FDT `/memory`
//! regions, minus the kernel image and the loaded DTB span. Single-CPU early
//! boot only — no locks.
//!
//! `allocPage` returns 0 on failure (never a valid page) to avoid Zig optional
//! ABI emitting 16-byte NEON stores that Data-Abort on misaligned slots.
...
/// Returns physical page address, or 0 if empty.
pub fn allocPage() usize {
    ...
    // Byte clear — avoid @memset NEON paths on unaligned/edge cases.
    var i: usize = 0;
